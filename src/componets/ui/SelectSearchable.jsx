  ...
  return (
    <div className="relative w-full">
      ...
      <input
        ...
        className=" ka-seclect block w-full rounded-md border border-gray-300 bg-white px-5 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
      />
      
      {/* 2. Lista suspensa de resultados (Renderização Condicional) */}
      {isOpen && (
        <ul className="absolute z-10 w-full mt-1 max-h-60 overflow-y-auto rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          {filteredOptions.length > 0 ? (
            filteredOptions.map((option) => (
              <li
                key={option.value}
                onClick={() => handleSelect(option.value)}
                className={`cursor-pointer colorfont-black px-3 py-2 text-sm hover:bg-gray-100 ${option.value === value ? 'bg-blue-50' : ''}`}
              >
                {option.label}
              </li>
            ))
